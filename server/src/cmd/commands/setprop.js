				...
				switch (prop.toLowerCase()) {
					...
					case "pquota":
						if (!validateQuotaString(args[1])) return client.sendMessage({
							...
							text: `Invalid value. Usage: /${this.data.usage}`
						});
						valueToSet = args[1];
						break;
					case "motd":
					...
					case "bgcolor":
						...
						if (valueToSet === false) return client.sendMessage({
							...
							text: `Invalid value. Must be a hex color.`
						});
						break;
					...
					case "stickyimagesize":
						...
						if (!(sizeCheck[0] > 0 && sizeCheck[1] > 0)) return client.sendMessage({
							...
							text: `Invalid value. Must be formatted as WIDTHxHEIGHT, and be greater than 0.`
						});
					default:
						valueToSet = args[1];
						break;
